...
- **VG arithmetic codegen bug `CONST + (VAR * CONST)` → `left + left` (OPEN, found Sep 2026)** — a binary `+` whose LEFT operand is a constant/literal and whose RIGHT operand is a multiplication mis-evaluates: `1024 + (srow * 40)` yields `2048` (=1024+1024) for all `srow`. Commutative-order-dependent: `(srow * 40) + 1024` and `srow * 40 + 1024` are correct; only `CONST + <multiply>` is wrong. Almost certainly a temp-slot clobber in `compile_expression`'s BINARY_OP path. Workaround: put the multiply on the left, or split into two statements. Separate from the jump-table fix. Full details + exact trigger table: `/memories/repo/build_and_test.md`.
