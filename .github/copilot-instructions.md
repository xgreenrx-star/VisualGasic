...
- **C64/GBA Emulator `BlitImage` no-op — FIXED Jul 30 2026** — both `demos/C64_Emulator/c64_main.vg` and `demos/GBA_Emulator/gba_main.vg` called `BlitImage` with `Rect2`/`Vector2` (float) instead of the required `Rect2i`/`Vector2i`/plain-int form, so the VIC/GPU framebuffer was never actually copied to the display texture (silent no-op, zero pixels copied, every frame, forever). Fixed by switching to `BlitImage`'s plain-integer 8-arg overload.
...
- **C64 Emulator boot never reaches "READY." — OPEN, root-caused Jul 30 2026, NOT a render bug** — this is a real interpreter-performance issue, not a rendering defect. KERNAL's RAMTAS RAM-test (PC `$FD50-$FD9C`, confirmed via ROM disassembly) legitimately writes test bytes through the VIC-II I/O page while sizing RAM — correct hardware behavior, imperceptible on real hardware but stretched into a long visible glitch because the emulator runs at only ~600-700 cycles/sec (real PAL hardware: ~985,000 cycles/sec, ~1000-1600x faster). Increasing `chunkCycles` (cycles processed per `_Process()` call) in `c64_main.vg` from 200→5000 gave NO throughput improvement — confirmed the bottleneck is genuine per-instruction AST-interpretation cost (`C64_Step()`/`Vic_Tick()` via the slow tree-walk fallback for cross-module Subs, see `/memories/repo/v6.0_blockers.md` §0), not `_Process()` call overhead. At current speed, clearing RAMTAS alone is estimated to take 20-30+ minutes of real time. Fixing this requires addressing the underlying bytecode-fallback/AST-interpretation-speed issue (see `/memories/repo/vg_bytecode_perf.md`), which is a separate, larger performance task. Full details: `/memories/repo/c64_vic_render_bug.md`.
