...
## Understanding Form Windows

### Forms are OS Windows

In Visual Gasic, each Form is built on Godot's `Window` node, which means **Forms appear as separate operating system windows** - just like in VB6 and WinForms. This is intentional to match the classic Visual Basic development experience.

**Key points:**

| Scenario | Behavior |
|----------|----------|
| Form is the **main scene** | Appears as the main application window |
| Form is **spawned from another scene** | Appears as a separate popup/dialog window |
| Form uses `ShowDialog()` | Appears as a modal dialog |
| Game runs in editor (F5) | Form opens in its own window alongside the game |

### When to Use Forms vs. Controls

| Use Forms (Window nodes) when: | Use Controls when: |
|-------------------------------|-------------------|
...
If you want a Form to render **inside** its parent window instead of as a separate OS window, you can set the `embedded` property:

```vb
' In Form_Load or InitializeComponent
Sub Form_Load()
    ' Render inside parent instead of separate window
    Me.embedded = True
End Sub
```

Or in the Inspector, select the Window node and enable **Embedded** under the Window section.

### Main Scene Configuration

To make a Form your main application window (no popup behavior):

1. Go to **Project → Project Settings**
2. Navigate to **Application → Run**  
3. Set **Main Scene** to your Form's `.tscn` file (e.g., `res://Form1.tscn`)

Now when you run the project, your Form will be the main window.
