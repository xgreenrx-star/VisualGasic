...
### Watch Window
A dedicated panel for monitoring variable values:
- Add expressions to watch list
- Auto-update when paused at breakpoint
- Color-coded value changes
- Persist watch list between sessions

### Conditional Breakpoints
Enhanced breakpoints with expressions:
- Right-click to add condition (e.g., `counter > 10`)
- Hit count conditions (break after N hits)
- Log message without breaking (tracepoints)

### Call Stack Panel
Visual call stack display:
- Show full stack when paused
- Click to navigate to any frame
- Display local variables per frame
...
- `ROADMAP.md` - Full development roadmap for upcoming features
