...
func setup_toolbox(attempts: int = 40):
	# Try to instantiate the native VisualGasicToolbox; it may not be registered yet
	# Remove temporary error label if present
	for child in toolbox.get_children():
		if child is Label and child.text == "VisualGasicToolbox Missing!":
			child.queue_free()
			break

	# Diagnostic: check several native class registrations
	var has_toolbox = ClassDB.class_exists("VisualGasicToolbox")
	var has_toolbutton = ClassDB.class_exists("VisualGasicToolButton")
	var has_syntax = ClassDB.class_exists("VisualGasicSyntaxHighlighter")
	var has_language = ClassDB.class_exists("VisualGasicLanguage")
	print("VisualGasic: ClassDB checks -> Toolbox:%s ToolButton:%s Syntax:%s Language:%s" % [str(has_toolbox), str(has_toolbutton), str(has_syntax), str(has_language)])

	if has_toolbox:
		# Try to instantiate and catch errors
		var ok = false
		var err_msg = ""
		var real_toolbox = ClassDB.instantiate("VisualGasicToolbox")
		if real_toolbox == null:
			print("VisualGasic: VisualGasicToolbox instantiation returned null")
		...
		if real_toolbox:
			...
			print("VisualGasic: Native Toolbox instantiated successfully")
			# Diagnostic: list toolbox tabs and tools
			for child in real_toolbox.get_children():
				print("VisualGasic: native toolbox tab -> %s (%s)" % [child.name, child.get_class()])
				for btn in child.get_children():
					var tt = "<no tooltip>"
					if btn.has_method("get_tooltip_text"):
						tt = str(btn.get_tooltip_text())
					print("VisualGasic:   tool -> %s class=%s tooltip=%s" % [btn.name, btn.get_class(), tt])
			return true

	# Show a temporary message and retry with exponential backoff to allow native module registration to complete
	var err = Label.new()
	err.text = "VisualGasicToolbox Missing!"
	# Avoid adding duplicate labels
	if not toolbox.get_children().size() or (toolbox.get_children().size() and not (toolbox.get_child(0) is Label and toolbox.get_child(0).text == err.text)):
		toolbox.add_child(err)

	if attempts > 0:
		var delay = 0.25 + (40 - attempts) * 0.1
		var t = get_tree().create_timer(delay)
		t.timeout.connect(Callable(self, "setup_toolbox").bind(attempts - 1))
		print("VisualGasic: Retry setup_toolbox in %s seconds (attempts left: %d)" % [str(delay), attempts - 1])
		return false

	print("VisualGasic: setup_toolbox giving up after retries")
	return false
