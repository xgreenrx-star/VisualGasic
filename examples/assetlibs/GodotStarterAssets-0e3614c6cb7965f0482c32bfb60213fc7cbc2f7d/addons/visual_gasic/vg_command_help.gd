...
static func _build_db() -> void:
	...
	_add("Global",
		...
		"Global highScore As Integer\nGlobal currentLevel As Integer = 1", 7919)
	...
	_add("Set",
		...
		"Dim player As Object\nSet player = New Player\nSet player = Nothing  ' Release reference", 12277)
	...
	_add("If",
		...
		"If score > highScore Then\n    highScore = score\n    Print \"New high score!\"\nElseIf score > 0 Then\n    Print \"Good job!\"\nElse\n    Print \"Try again!\"\nEnd If\n\n' Single-line form:\nIf health <= 0 Then gameOver = True", 8182)
	...
	_add("New",
		...
		"Dim player As New Player\nDim enemies As New Collection\n\nSet boss = New Boss(\"Dragon\", 500)", 9940)
	...
	_add("Enum",
		...
		"Enum GameState\n    Menu = 0\n    Playing = 1\n    Paused = 2\n    GameOver = 3\nEnd Enum\n\nDim state As GameState = GameState.Playing", 7374)
	...
	_add("Print",
		...
		"Print \"Score: \" & score\nPrint \"X=\"; x; \" Y=\"; y\nPrint #1, \"Log entry: \" & message", 10720)
	...
	_add("Close",
		...
		"Open \"data.txt\" For Input As #1\n' ... read data ...\nClose #1\n\nClose  ' Close all open files", 5768)
	...
	_add("Transform3D",
		...
		"Dim tr = Transform3D(Basis(), Vector3(0, 2, 5))\ncam.transform = tr.LookingAt(player.position, Vector3(0, 1, 0))", 13982)
	...
	_add("Camera.Zoom",
		...
		"Camera.Zoom Vector2(2, 2)       ' 2x zoom in\nCamera.Zoom 0.5                  ' zoom out to half", 5382)
	...
	_add("Camera.MakeCurrent",
		...
		"Camera.MakeCurrent cutsceneCam\n' ... play cutscene ...\nCamera.MakeCurrent gameplayCam", 5226)
	...
	_add("Camera.Rotation",
		...
		"' Quick screen tilt (Camera2D)\nCamera.Rotation 0.1   ' ~6 degrees", 5321)
	...
	_add("Sound.Seek",
		...
		"Sound.Seek music, 30.0   ' jump to 30 seconds in", 12923)
	...
	_add("Sound.Position",
		...
		"Dim t = Sound.Position(music)\nPrint \"At \" & Round(t, 1) & \" seconds\"", 12866)
	...
	_add("Speaker.Solo",
		...
		"Speaker.Solo \"SFX\", True   ' only sound effects audible", 13178)
	...
	_add("Speaker.Name",
		...
		"Print Speaker.Name(0)   ' usually \"Master\"", 13150)
	...
	_add("Animation.Play",
		...
		"Animation.Play playerAnim, \"walk\"\nAnimation.Play playerAnim, \"sprint\", 1.5", 4400)
	...
	_add("Animation.Seek",
		...
		"Animation.Seek playerAnim, 1.5", 4459)
	...
	_add("Animation.Speed",
		...
		"Animation.Speed playerAnim, 0.5   ' slow motion", 4489)
	...
	_add("Physics.Impulse",
		...
		"Physics.Impulse ball, Vector2(500, -200)   ' kick the ball", 10570)
	...
	_add("Nav.SetTarget",
		...
		"Nav.SetTarget enemyNav, player.Position", 9911)
	...
	_add("Screen.Width",
		...
		"If Screen.Width() < 600 Then SetMobileUI()", 11991)

	_add("Screen.Height",
		"Screen.Height() As Long",
		"Returns the screen height in pixels.",
		"Print Screen.Width() & \"x\" & Screen.Height()", 11889)

	_add("Screen.DPI",
		"Screen.DPI() As Long",
		"Returns the screen DPI (dots per inch). Useful for sizing UI on high-density displays.",
		"Dim scale = Screen.DPI() / 96.0   ' 1.0 on standard displays", 11837)
	...
	_add("Joypad.Button",
		...
		"If Joypad.Button(0, 0) Then Jump()", 8900)
	...
	_add("Sensor.Units",
		...
		"Sensor.Units \"game\"     ' default, easy mode\nSensor.Units \"metric\"   ' physics-accurate", 12248)
	...
	_add("Sensor.Tilt",
		...
		"player.Velocity.x = Sensor.Tilt() * 10   ' tilt-to-steer", 12224)
	...
	_add("Permission.Has",
		...
		"If Not Permission.Has(\"camera\") Then Permission.Request \"camera\"", 10365)
	...
	_add("Permission.Request",
		...
		"Permission.Request \"location\"", 10393)
	...
	_add("Vibrate",
		...
		"Vibrate 100           ' short buzz\nVibrate 500, 0.3      ' half-second gentle", 14386)
	...
	_add("Crypto.RandomBytes",
		...
		"Dim token = RandomBytes(32)\nDim hex = Base64Encode(token)", 6334)
	...
	_add("Crypto.Base64Encode",
		...
		"Print Base64Encode(\"hello world\")  ' aGVsbG8gd29ybGQ=", 6192)
	...
	_add("Crypto.Base64Decode",
		...
		"Print Base64Decode(\"aGVsbG8=\")  ' hello\nDim bin = Base64Decode(encoded, True)", 6162)
	...
	_add("Bone.LookAt",
		...
		"Bone.LookAt skel, headIdx, player.GlobalPosition", 4732)
	...
	_add("ImageWidth",
		...
		"Dim img = CreateImage(320, 240)\nPrint ImageWidth(img)   ' 320\nPrint ImageHeight(img)  ' 240", 8329)
	...
	_add("RGB",
		...
		"Dim c As Variant = RGB(255, 0, 0)  ' Red\nDrawRect 0, 0, 100, 100, RGB(0, 128, 255)  ' Sky blue", 11680)
	...
	_add("Camera.PanTo",
		...
		"' Cinematic reveal\nCamera.PanTo Vector2(800, 400), 1.5", 5256)
	...
	_add("Camera.Bounce",
		...
		"' Shotgun recoil\nCamera.Bounce Vector2(-1, 0), 18", 5066)
	...
	_add("Camera.FlashColor",
		...
		"' Damage flash\nCamera.FlashColor RGB(255, 0, 0), 0.15", 5097)
	...
	_add("Animation.Loop",
		...
		"Animation.Loop \"idle\", True\nAnimation.Loop \"jump\", False", 4341)
	...
	_add("Physics.Gravity",
		...
		"Physics.Gravity 980        ' classic down\nPhysics.Gravity Vector2(0, -980)  ' anti-gravity zone", 10482)
	...
	_add("Ray.Cast2D",
		...
		"Dim hit = Ray.Cast2D(player.Position, Mouse.Position)\nIf Not hit.is_empty() Then Print hit.collider.name", 11186)
	...
	_add("Ray.Cast3D",
		...
		"Dim hit = Ray.Cast3D(cam.GlobalPosition, cam.GlobalPosition + cam.Basis.z * -100)", 11217)
	...
	_add("Joypad.IsConnected",
		...
		"If Joypad.IsConnected(0) Then ShowPlayerJoinedIcon()", 8957)
	...
	_add("Joypad.Stick",
		...
		"Dim move = Joypad.Stick(0, 0)\nplayer.Velocity = move * speed", 9013)
	...
	_add("Sensor.Magnetometer",
		...
		"Dim compass = Sensor.Magnetometer()", 12200)
	...
	_add("Crypto.Hex",
		...
		"Print Crypto.Hex(Crypto.RandomBytes(8))   ' e.g. \"a1b2c3d4e5f60718\"", 6248)
	...
	_add("Theme.Get",
		...
		"Dim panelBg = Theme.Get(myPanel, \"color\", \"bg_color\")", 13680)
	...
	_add("Speaker.Bus",
		...
		"Speaker.Bus.Volume \"Master\", 75", 13010)
