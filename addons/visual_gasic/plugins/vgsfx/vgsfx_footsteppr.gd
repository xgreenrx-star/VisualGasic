# VGSFX Footsteppr — physical-simulation footstep synth.
#
...
@tool
class_name VGSFXFootsteppr
extends RefCounted

const VGSFXPD_ := preload("res://addons/visual_gasic/plugins/vgsfx/vgsfx_pd.gd")
const VGSFXPDCompile_ := preload("res://addons/visual_gasic/plugins/vgsfx/vgsfx_pd_compile.gd")
const VGSFXPDModules_ := preload("res://addons/visual_gasic/plugins/vgsfx/vgsfx_pd_modules.gd")
...
func _get_terrain_program(terrain_name: String) -> Callable:
	...
	var modules: Dictionary = VGSFXPDModules_.MODULES
	if not modules.has(terrain_name):
		push_error("VGSFXFootsteppr: unknown terrain '%s'" % terrain_name)
		return Callable()
	var src: String = modules[terrain_name]
	var fn: Callable = VGSFXPDCompile_.compile(src)
...
func generate_sound() -> PackedFloat32Array:
	...
	VGSFXPD_.set_stream_length_seconds(step_length)

	var heel_env: Callable = VGSFXPD_.resize_fn(VGSFXPD_.make_step(step_heel), 0.0, 1.0, 0.0, 0.3333)
	var roll_env: Callable = VGSFXPD_.resize_fn(VGSFXPD_.make_step(step_roll), 0.0, 1.0, 0.125, 0.875)
	var ball_env: Callable = VGSFXPD_.resize_fn(VGSFXPD_.make_step(step_ball), 0.0, 1.0, 0.6667, 1.0)
	var step_env_0_1: Callable = VGSFXPD_.add_fns([heel_env, roll_env, ball_env])
	var step_env_resized: Callable = VGSFXPD_.resize_fn(step_env_0_1, 0.0, 1.0, 0.0, step_length)
	var envelope_signal: PackedFloat32Array = VGSFXPD_.pd_fn(step_env_resized)
...
func to_audio_stream_wav() -> AudioStreamWAV:
	...
	stream.mix_rate = VGSFXPD_.SAMPLE_RATE
	stream.stereo = false
	stream.data = pcm
	return stream
