# VGSFX PureData mini-interpreter.
#
...
static func compile(src: String) -> Callable:
	...
	for i in range(function_calls.size()):
		if String(function_calls[i][0]) == "outlet~":
			...
			break
	if output_idx == -1:
		push_error("VGSFXPDCompile: no outlet~ found")
		return Callable()
...
static func _build_program(function_calls: Array, connections: Array, output_node_idx: int) -> Array:
	...
	for nid_v in ordered_ids:
		...
		if not FUNCTION_INFO.has(nname):
			push_error("VGSFXPDCompile: unknown pd object '%s'" % nname)
			continue
...
static func _run_program(program: Array, envelope_signal: PackedFloat32Array) -> PackedFloat32Array:
	...
	for step in program:
		...
		for slot in range(inputs.size()):
			...
			if srcs.is_empty():
				if params[slot] == null:
					args.append(VGSFXPD_.pd_c(0.0))
				else:
					args.append(VGSFXPD_.pd_c(float(params[slot])))
			elif srcs.size() == 1:
				args.append(slots.get(int(srcs[0]), VGSFXPD_.pd_c(0.0)))
			else:
				var bufs := []
				for s in srcs:
					bufs.append(slots.get(int(s), VGSFXPD_.pd_c(0.0)))
				args.append(VGSFXPD_.pd_polyadd(bufs))

		var result: PackedFloat32Array
		match op:
			"inlet":   result = envelope_signal
			"outlet":  result = args[0]
			"mul":     result = VGSFXPD_.pd_mul(args[0], args[1])
			"div":     result = VGSFXPD_.pd_div(args[0], args[1])
			"add":     result = VGSFXPD_.pd_add(args[0], args[1])
			"c":       result = args[0]
			"noise":   result = VGSFXPD_.pd_noise()
			"osc":     result = VGSFXPD_.pd_osc(args[0])
			"clip":    result = VGSFXPD_.pd_clip(args[0], args[1], args[2])
			"sqrt":    result = VGSFXPD_.pd_sqrt(args[0])
			"lop":     result = VGSFXPD_.pd_lop(args[0], args[1])
			"hip":     result = VGSFXPD_.pd_hip(args[0], args[1])
			"bp":      result = VGSFXPD_.pd_bp(args[0], args[1], args[2])
			"vcf":     result = VGSFXPD_.pd_vcf(args[0], args[1], args[2])
			"noop":    result = VGSFXPD_.pd_c(0.0)
			_:
				push_error("VGSFXPDCompile: unknown op '%s'" % op)
				result = VGSFXPD_.pd_c(0.0)
