...
# ─ Scene root selector ───────────────────────────────────────────────────────
var _root_selector_popup: Window = null
var _root_selector_option: OptionButton = null
var _pending_scene_export_is_3d: bool = false
var _pending_scene_export_data: Dictionary = {}

# ─ Import VG dialog ──────────────────────────────────────────────────────────
var _import_vg_dialog: FileDialog = null

# ─ Open-after-export ─────────────────────────────────────────────────────────
var _last_exported_vg_path: String = ""
var _last_exported_tscn_path: String = ""

# ─ Fallback breakpoints & validator ─────────────────────────────────────────
var _fallback_breakpoints: Dictionary = {}  # node_name -> bool
var _fallback_node_bp_menu: PopupMenu = null  # right-click extension for bp
...
func _build_inline_fallback(host: Control, reason: String) -> void:
	...
	row2.add_child(VSeparator.new())
	# Execution order badge (feature 13) – now animated
	_wn_btn(row2, "⓪ Order", _fallback_show_exec_order_animated)
	row2.add_child(VSeparator.new())
	...
	# ── Toolbar row 4: Code generation / Import ───────────────────────────
	var row4 := HBoxContainer.new()
	row4.add_theme_constant_override("separation", 4)
	root.add_child(row4)
	var r4_lbl := Label.new(); r4_lbl.text = "Export/Import:"
	r4_lbl.add_theme_color_override("font_color", Color(1.0, 0.9, 0.5))
	row4.add_child(r4_lbl)
	_wn_btn(row4, "▶ VG",       _fallback_export_vg)
	_wn_btn(row4, "⬡ 2D Scene", _fallback_export_scene_2d)
	_wn_btn(row4, "◈ 3D Scene", _fallback_export_scene_3d)
	_wn_btn(row4, "GD Script",  _fallback_export_gdscript)
...
func _fallback_delete_selected() -> void:
	...
	if _fallback_undo_redo != null:
		...
		for nd in to_del:
			var nname: StringName = nd.name
			var nkind: String = _fallback_node_meta.get(nname, "Event")
			var npos: Vector2 = nd.position_offset
...
func _on_graph_node_selected_fallback(node: Node) -> void:
	if node is GraphNode:
		...
		if _fallback_node_menu and is_instance_valid(_fallback_node_menu):
			var mp: Vector2 = _fallback_graph.get_global_rect().position + node.position_offset - _fallback_graph.scroll_offset
...
func _on_node_menu_id_pressed(id: int) -> void:
	...
	match id:
		...
		1:  # Delete
			...
			_fallback_breakpoints.erase(_fallback_last_selected_node.name)
		...
		3:  # Toggle Breakpoint
			var nd := _fallback_last_selected_node
			var cur: bool = _fallback_breakpoints.get(nd.name, false)
			_fallback_breakpoints[nd.name] = not cur
			# Update menu label on next open
			if _fallback_node_menu and is_instance_valid(_fallback_node_menu):
				_fallback_node_menu.set_item_text(3,
					"🔴 Remove Breakpoint" if not cur else "🔴 Set Breakpoint")
			# Visual indicator: red border when active
			if not cur:
				var sty := StyleBoxFlat.new()
				sty.bg_color = nd.modulate.darkened(0.5)
				sty.border_width_left = 3; sty.border_width_top = 3
				sty.border_width_right = 3; sty.border_width_bottom = 3
				sty.border_color = Color(1.0, 0.2, 0.2); sty.set_corner_radius_all(6)
				nd.add_theme_stylebox_override("panel", sty)
			else:
				nd.remove_theme_stylebox_override("panel")
			_fallback_set_status("Breakpoint %s on '%s'." % ["set" if not cur else "removed", nd.title])
...
func _fallback_add_node(node_kind: String, at_pos: Vector2 = Vector2(-1, -1)) -> GraphNode:
	...
	if node_kind == "Note":
		...
		node.add_child(ta)
		# ─ Sticky-note background color picker ─
		var note_color_row := HBoxContainer.new()
		var note_color_lbl := Label.new(); note_color_lbl.text = "Tint:"
		note_color_lbl.add_theme_color_override("font_color", Color(0.85, 0.75, 0.4))
		note_color_row.add_child(note_color_lbl)
		var note_cpb := ColorPickerButton.new()
		note_cpb.color = Color(0.28, 0.26, 0.12)
		note_cpb.custom_minimum_size = Vector2(56, 22)
		note_cpb.color_changed.connect(func(c: Color):
			var s := StyleBoxFlat.new()
			s.bg_color = c
			s.border_width_left = 3; s.border_color = c.lightened(0.35)
			node.add_theme_stylebox_override("panel", s)
		)
		note_color_row.add_child(note_cpb)
		node.add_child(note_color_row)
...
func _connect_editor_export_signals() -> void:
	if _editor == null or not is_instance_valid(_editor):
		return
	if _editor.has_signal("export_vg_requested") and \
			not _editor.is_connected("export_vg_requested", _on_export_vg):
		_editor.connect("export_vg_requested", _on_export_vg)
	if _editor.has_signal("export_scene_2d_requested") and \
			not _editor.is_connected("export_scene_2d_requested", _on_export_scene_2d):
		_editor.connect("export_scene_2d_requested", _on_export_scene_2d)
...
func _on_export_vg(data: Dictionary) -> void:
	var stem := ""
	if _editor != null and is_instance_valid(_editor) and _editor.has_method("get_export_stem"):
		stem = _editor.call("get_export_stem")
	if stem.is_empty():
		stem = "working_nodes_export"
	var vg_path := "res://" + stem + ".vg"
	var code: String = working_nodes_codegen.generate_vg_code(data)
	var f := FileAccess.open(vg_path, FileAccess.WRITE)
	if f:
		f.store_string(code)
		f.close()
		_fallback_set_status("VG exported → " + vg_path)
		_show_export_success_dialog(vg_path, "")
	else:
		_fallback_set_status("Export failed: cannot write " + vg_path)


func _on_export_scene_2d(data: Dictionary) -> void:
	_show_scene_root_selector(false, data)
...
func _fallback_collect_data() -> Dictionary:
	if _fallback_graph == null or not is_instance_valid(_fallback_graph):
		return {"version": 1, "nodes": [], "connections": [], "groups": []}
	var nodes_arr: Array = []
	for c in _fallback_graph.get_children():
		if c is GraphNode:
			nodes_arr.append({
				"name": c.name,
				"title": c.title,
				"kind": _fallback_node_meta.get(c.name, "Event"),
				"type": _fallback_node_meta.get(c.name, "Event").to_lower(),
				"position": [c.position_offset.x, c.position_offset.y],
			})
	var conns_arr: Array = []
	for conn in _fallback_graph.get_connection_list():
		conns_arr.append({
	...
	return {"version": 1, "nodes": nodes_arr, "connections": conns_arr, "groups": []}


func _fallback_export_vg() -> void:
	var stem := _fallback_save_path.get_basename().get_file()
	if stem.is_empty() or stem == "working_nodes_fallback":
		stem = "working_nodes_export"
	var vg_path := "res://" + stem + ".vg"
	var code: String = working_nodes_codegen.generate_vg_code(_fallback_collect_data())
	var f := FileAccess.open(vg_path, FileAccess.WRITE)
	if f:
		f.store_string(code)
		f.close()
...
func _fallback_export_scene_2d() -> void:
	_show_scene_root_selector(false, _fallback_collect_data())


func _fallback_export_scene_3d() -> void:
	_show_scene_root_selector(true, _fallback_collect_data())


func _write_export_files(vg_path: String, vg_code: String, tscn_path: String, tscn: String) -> void:
	var ok_vg := false
	var f1 := FileAccess.open(vg_path, FileAccess.WRITE)
	if f1:
		f1.store_string(vg_code)
		f1.close()
		ok_vg = true
	var ok_tscn := false
	var f2 := FileAccess.open(tscn_path, FileAccess.WRITE)
	if f2:
		f2.store_string(tscn)
		f2.close()
		ok_tscn = true
	if ok_vg and ok_tscn:
		_last_exported_vg_path = vg_path
		_last_exported_tscn_path = tscn_path
		_fallback_set_status("Exported → %s  +  %s" % [vg_path, tscn_path])
		_show_export_success_dialog(vg_path, tscn_path)
	else:
		_fallback_set_status("Export partially failed (vg=%s tscn=%s)" % [str(ok_vg), str(ok_tscn)])
...
func _fallback_export_gdscript() -> void:
	var stem := _fallback_save_path.get_basename().get_file()
	if stem.is_empty() or stem == "working_nodes_fallback":
		stem = "working_nodes_export"
	var gd_path := "res://" + stem + ".gd"
	var code: String = working_nodes_codegen.generate_gdscript(_fallback_collect_data())
	var f := FileAccess.open(gd_path, FileAccess.WRITE)
	if f:
		f.store_string(code)
		f.close()
		_fallback_set_status("GDScript exported → " + gd_path)
		_show_export_success_dialog(gd_path, "")
	else:
		_fallback_set_status("GDScript export failed: cannot write " + gd_path)


# ══════════════════════════════════════════════════════════════════════════════
# Scene root-type selector (before 2D / 3D scene export)
# ══════════════════════════════════════════════════════════════════════════════

func _show_scene_root_selector(is_3d: bool, data: Dictionary) -> void:
	_pending_scene_export_is_3d = is_3d
	_pending_scene_export_data = data
	if _root_selector_popup == null or not is_instance_valid(_root_selector_popup):
		_root_selector_popup = Window.new()
		_root_selector_popup.title = "Choose Scene Root Type"
		_root_selector_popup.min_size = Vector2(320, 160)
		var vb := VBoxContainer.new()
		_root_selector_popup.add_child(vb)
		var lbl := Label.new()
		...
		vb.add_child(ok_btn)
		_view.add_child(_root_selector_popup)
	_root_selector_option.clear()
	if is_3d:
		for t in ["Node3D", "Spatial", "MeshInstance3D", "CharacterBody3D",
				  "RigidBody3D", "Area3D", "StaticBody3D", "VehicleBody3D"]:
			_root_selector_option.add_item(t)
	else:
		for t in ["Node2D", "CanvasLayer", "Control", "CharacterBody2D",
				  "RigidBody2D", "Area2D", "StaticBody2D", "TileMap"]:
			_root_selector_option.add_item(t)
	_root_selector_popup.popup_centered()

func _on_root_selector_confirmed() -> void:
	_root_selector_popup.hide()
	var root_type: String = _root_selector_option.get_item_text(_root_selector_option.selected)
	var stem := _fallback_save_path.get_basename().get_file()
	if stem.is_empty() or stem == "working_nodes_fallback":
		stem = "working_nodes_export"
	var vg_path := "res://" + stem + ".vg"
	...
	var tscn: String
	var code: String = working_nodes_codegen.generate_vg_code(_pending_scene_export_data)
	if _pending_scene_export_is_3d:
		tscn_path = "res://" + stem + "_3d.tscn"
		tscn = working_nodes_codegen.generate_scene_3d_tscn(_pending_scene_export_data, vg_path, root_type)
	else:
		tscn_path = "res://" + stem + "_2d.tscn"
		tscn = working_nodes_codegen.generate_scene_2d_tscn(_pending_scene_export_data, vg_path, root_type)
	_write_export_files(vg_path, code, tscn_path, tscn)


# ══════════════════════════════════════════════════════════════════════════════
# Open-after-export success dialog
# ══════════════════════════════════════════════════════════════════════════════

func _show_export_success_dialog(vg_path: String, tscn_path: String) -> void:
	...
	if vg_path != "":
		var lbl := Label.new()
		lbl.text = "Exported: " + vg_path
		lbl.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		vb.add_child(lbl)
	if tscn_path != "":
		var lbl2 := Label.new()
		lbl2.text = "Scene: " + tscn_path
		lbl2.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
		vb.add_child(lbl2)
	# Open-in-editor buttons (only available in Godot editor context)
	if Engine.is_editor_hint() and ClassDB.class_exists("EditorInterface"):
		if vg_path != "":
			var open_vg := Button.new()
			open_vg.text = "Open VG in Code Editor"
			open_vg.pressed.connect(func():
				EditorInterface.edit_resource(load(vg_path))
				d.queue_free()
			)
			vb.add_child(open_vg)
		if tscn_path != "":
			var open_sc := Button.new()
			open_sc.text = "Open Scene"
			open_sc.pressed.connect(func():
				EditorInterface.open_scene_from_path(tscn_path)
				d.queue_free()
			)
			vb.add_child(open_sc)
	_view.add_child(d)
	d.popup_centered(Vector2(420, 220))
	d.confirmed.connect(func(): d.queue_free())
	d.canceled.connect(func(): d.queue_free())


# ══════════════════════════════════════════════════════════════════════════════
# Import VG → load parsed graph data into fallback GraphEdit
# ══════════════════════════════════════════════════════════════════════════════

func _show_import_vg_dialog() -> void:
	if _import_vg_dialog == null or not is_instance_valid(_import_vg_dialog):
		_import_vg_dialog = FileDialog.new()
		_import_vg_dialog.file_mode = FileDialog.FILE_MODE_OPEN_FILE
		_import_vg_dialog.access = FileDialog.ACCESS_FILESYSTEM
		_import_vg_dialog.filters = PackedStringArray(["*.vg ; VG Script Files"])
		_import_vg_dialog.title = "Import VG File"
		_import_vg_dialog.min_size = Vector2(600, 400)
		_import_vg_dialog.file_selected.connect(_on_import_vg_file_selected)
		_view.add_child(_import_vg_dialog)
	_import_vg_dialog.popup_centered()

func _on_import_vg_file_selected(path: String) -> void:
	var f := FileAccess.open(path, FileAccess.READ)
	if not f:
		_fallback_set_status("Cannot read: " + path)
		return
	var vg_code := f.get_as_text()
	f.close()
	var data: Dictionary = working_nodes_codegen.parse_vg_to_graph_data(vg_code)
	if data.is_empty() or not data.has("nodes"):
		_fallback_set_status("Import failed: no graph data parsed from " + path)
		return
	_fallback_load_parsed_data(data)
	_fallback_set_status("Imported %d node(s) from %s" % [data["nodes"].size(), path.get_file()])

func _fallback_load_parsed_data(data: Dictionary) -> void:
	if _fallback_graph == null or not is_instance_valid(_fallback_graph):
		return
	...
	for c in _fallback_graph.get_children():
		if c is GraphNode:
			c.queue_free()
	_fallback_node_meta.clear()
	_fallback_next_id = 1
	await _fallback_graph.get_tree().process_frame  # let queue_free execute

	# Place nodes
	var name_remap: Dictionary = {}
	for nd in data.get("nodes", []):
		var kind: String = str(nd.get("type", "Action")).capitalize()
		var new_node := _fallback_add_node(kind)
		if new_node == null: continue
		new_node.title = str(nd.get("title", kind))
		var pos_a: Array = nd.get("position", [200, 200])
		new_node.position_offset = Vector2(float(pos_a[0]), float(pos_a[1]))
		name_remap[str(nd.get("name", ""))] = new_node.name
		if nd.get("_breakpoint", false):
			_fallback_breakpoints[new_node.name] = true

	# Recreate connections
	for c in data.get("connections", []):
		var fn := StringName(name_remap.get(str(c.get("from", "")), ""))
		var tn := StringName(name_remap.get(str(c.get("to", "")), ""))
		if fn != "" and tn != "":
			_fallback_graph.connect_node(fn, int(c.get("from_port", 0)), tn, int(c.get("to_port", 0)))
	_fallback_update_node_count()


# ══════════════════════════════════════════════════════════════════════════════
# Fallback graph validator
# ══════════════════════════════════════════════════════════════════════════════

func _fallback_validate_graph() -> void:
	if _fallback_graph == null or not is_instance_valid(_fallback_graph):
		return
	var issues: Array = []
	var has_event := false
	var connected_to: Dictionary = {}
	for c in _fallback_graph.get_connections():
		connected_to[str(c.get("to_node", ""))] = true

	for ch in _fallback_graph.get_children():
		if not (ch is GraphNode): continue
		var kind: String = _fallback_node_meta.get(ch.name, "")
		if kind == "Event":
			has_event = true
		elif kind == "Action" and not connected_to.has(ch.name):
			issues.append("Action '%s' has no incoming connection." % ch.title)

	if not has_event:
		issues.append("No Event node found — nothing will fire on execution.")

	var d := AcceptDialog.new()
	d.title = "Working Nodes — Validator"
	d.min_size = Vector2(440, 260)
	if issues.is_empty():
		d.dialog_text = "✔  No issues found. Graph looks good!"
	else:
		d.dialog_text = "Found %d issue(s):\n\n• " % issues.size() + "\n• ".join(issues)
	_view.add_child(d)
	d.popup_centered()
	d.confirmed.connect(func(): d.queue_free())
...
# ══════════════════════════════════════════════════════════════════════════════
# Execution order animated badges (fallback version)
# ══════════════════════════════════════════════════════════════════════════════

func _fallback_show_exec_order_animated() -> void:
	if _fallback_graph == null or not is_instance_valid(_fallback_graph):
		return
	# Clear old badges
	for ch in _fallback_graph.get_children():
		if ch is GraphNode and is_instance_valid(ch):
			var b: Node = ch.get_node_or_null("_exec_badge")
			if b: b.queue_free()

	# BFS from event nodes through graph connections
	var queue: Array = []
	for ch in _fallback_graph.get_children():
		if ch is GraphNode and _fallback_node_meta.get(ch.name, "") == "Event":
			queue.append(ch.name)
	var visited: Dictionary = {}
	var order: Dictionary = {}
	var counter := 1
	while not queue.is_empty():
		...
		counter += 1
		for c in _fallback_graph.get_connections():
			if str(c.get("from_node", "")) == cur:
				queue.push_back(str(c.get("to_node", "")))

	for nname in order:
		var nd: GraphNode = _fallback_graph.get_node_or_null(NodePath(nname))
...
# ══════════════════════════════════════════════════════════════════════════════
# Fallback copy / paste  (Ctrl+C / Ctrl+V on graph)
# ══════════════════════════════════════════════════════════════════════════════
...
func _fallback_copy_selected() -> void:
	if _fallback_graph == null: return
	var selected: Array = []
	for c in _fallback_graph.get_children():
		if c is GraphNode and c.selected:
			selected.append(c)
	...
	for n in selected:
		nodes_arr.append({
			...
			"kind": _fallback_node_meta.get(n.name, "Action"),
			"position": [n.position_offset.x, n.position_offset.y],
		})
	var conns_arr: Array = []
	for c in _fallback_graph.get_connections():
		if sel_names.has(str(c.get("from_node",""))) and sel_names.has(str(c.get("to_node",""))):
			conns_arr.append(c)
	_fallback_clipboard_data = JSON.stringify({"wn_clipboard": true, "nodes": nodes_arr, "connections": conns_arr})
	DisplayServer.clipboard_set(_fallback_clipboard_data)

func _fallback_paste() -> void:
	if _fallback_graph == null: return
	var raw := DisplayServer.clipboard_get()
	if raw.is_empty(): raw = _fallback_clipboard_data
	if raw.is_empty(): return
	var parsed = JSON.parse_string(raw)
	if not (parsed is Dictionary) or not parsed.get("wn_clipboard", false): return
	var remap: Dictionary = {}
	for nd in parsed.get("nodes", []):
		var kind: String = str(nd.get("kind", "Action")).capitalize()
		var n := _fallback_add_node(kind)
		if n == null: continue
		n.title = str(nd.get("title", kind)) + " (copy)"
		var pos_a: Array = nd.get("position", [200, 200])
		n.position_offset = Vector2(float(pos_a[0]) + 30, float(pos_a[1]) + 30)
		remap[str(nd.get("name", ""))] = n.name
	for c in parsed.get("connections", []):
		var fn := StringName(remap.get(str(c.get("from_node","")), ""))
		var tn := StringName(remap.get(str(c.get("to_node","")), ""))
		if fn != "" and tn != "":
			_fallback_graph.connect_node(fn, int(c.get("from_port",0)), tn, int(c.get("to_port",0)))
	_fallback_update_node_count()
...
func _style_wn_option_btn(btn: OptionButton) -> void:
	...
	if not popup.about_to_popup.is_connected(_on_wn_popup_about_to_show):
		popup.about_to_popup.connect(_on_wn_popup_about_to_show.bind(popup))
...
func _do_style_wn_popup(popup: PopupMenu) -> void:
	...
	for c: String in ["font_focus_color", "font_pressed_color", "font_selected_color"]:
		popup.add_theme_color_override(c, Color.BLACK)
