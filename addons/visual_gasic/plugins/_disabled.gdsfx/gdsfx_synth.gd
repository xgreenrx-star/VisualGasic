# GDSFX — Bfxr synth (port of bfxr2/js/synths/Bfxr.js + SynthBase.js).
#
...
func set_param(name: String, value, check_locked: bool = false) -> void:
	if not params.has(name):
		push_error("GDSFX: unknown param: " + name)
		return
...
func generate_sound() -> PackedFloat32Array:
	var dsp = GDSFXDSP_.new(params.duplicate(), self)
