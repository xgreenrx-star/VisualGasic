...
func generate_image(prompt: String, options: Dictionary = {}) -> Dictionary:
	var endpoint = _get_endpoint() + "/images/generations"
	var size = String(options.get("size", "512x512"))
	var body: Dictionary = {
		"prompt": prompt,
		"size": size,
	}
...
func _get_endpoint() -> String:
	var endpoint = String(_config.get("endpoint", "https://api.openai.com/v1"))
	return endpoint.strip_suffix("/")

func _get_model() -> String:
	return String(_config.get("model", "gpt-4.1-mini"))

func _build_headers() -> Array:
	var api_key = String(_config.get("api_key", ""))
	var headers: Array = [
