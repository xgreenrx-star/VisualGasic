...
- Task.Run / Parallel For / Parallel Section now backed by real `std::thread` with per-thread scope cloning
- VGSystem, VGSignalHandler, VGFilePermissions, VGMemoryBuffer, VGIPC, VGAndroidBridge provide full system-level programming
...
4. ✅ ~~**Mobile Platform Support**~~ - VGAndroidBridge (JNI device info, permissions, intents, toast, vibrate, battery, storage)
