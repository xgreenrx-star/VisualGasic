...
# ─── GD-style trigger / action node kinds ────────────────────────────────────
...
func _build_ui() -> void:
	...
	# ─ Toolbar row 1b: GD-style trigger node shortcuts ───────────────────────
	...
	for _gd_k in ["on_start", "move", "rotate", "scale", "alpha",
...
func _create_node_ui(node_type: String) -> GraphNode:
	...
	# ── GD-style dedicated trigger nodes — multi-port data-driven layout ─
...
# ══════════════════════════════════════════════════════════════════════════════
# GD trigger node builder (data-driven)
# ══════════════════════════════════════════════════════════════════════════════
...
func _build_trigger_node(n: GraphNode, def: Dictionary) -> void:
	...
	n.custom_minimum_size = Vector2(260, max(96, 74 + params.size() * 30))

	# ── Slot 0: execution-flow header ────────────────────────────────────
	...
	# ── Slots 1..N: parameter rows ───────────────────────────────────────
	for i in params.size():
		var p: Array = params[i]
		var row := _make_param_row(str(p[0]), str(p[1]) if p.size() > 1 else "")
		n.add_child(row)
		n.set_slot(1 + i, true, PORT_VALUE, VALUE_PORT_COLOR, false, 0, Color.WHITE)
	...
	n.add_child(gl)
	n.set_slot(1 + params.size(), false, 0, Color.WHITE, false, 0, Color.WHITE)
...
func _node_title_for_type(node_type: String, index: int) -> String:
	if _GD_TRIGGERS.has(node_type):
		return _GD_TRIGGERS[node_type].get("title", node_type.capitalize()) + " %d" % index
	match node_type:
		TYPE_EVENT:  return "Trigger %d" % index
		TYPE_ACTION: return "Action %d"  % index
		TYPE_MATH:   return "Math %d"    % index
		_:           return "Node %d"    % index
...
func _validate_graph() -> void:
	...
	for node_name in _nodes:
		...
		if ntype == TYPE_EVENT or ntype == TYPE_ON_START:
			has_event = true
...
const _ALL_NODE_KINDS: Array = [
	# ── GD-style dedicated trigger nodes ─────────────────────────────────────
	["▶ On Start",     "on_start"],
	["↔ Move",         "move"],
	["↻ Rotate",       "rotate"],
	["⤡ Scale",        "scale"],
...
func _on_palette_item_activated(index: int) -> void:
	...
	_palette_popup.hide()
	# GD trigger nodes and legacy base types handled uniformly
