...
2. ✅ **Encode-path literal typing (mitigated Sep 2026)** — VG bare numeric literals in `Array(...)` PyCall args arrived as Python `float` on the **JSON wire path**. **Mitigation shipped:** enable `vg/python/use_typed_protocol` (msgpack C2) so integers survive encode. JSON-default users can still use `CInt()`. Literal type syntax (`0i`) remains a v6.1 candidate. Large-array binary performance still Phase 1 remainder.
...
**v6.0 implementation (text-mode) — ✅ shipped Sep 2026:**
- C++ AST API: `VisualGasicLanguage.vg_analyze_causal_graph(code, roots)` (`src/visual_gasic_causal_graph.cpp`)
- GDScript wrapper: `vg_causal_chain.gd` (C++ preferred, regex fallback)
- Entry points: every `Sub <ControlName>_<EventName>()` in the file (or explicit `roots` array)
- Calls: every `Call` statement and `RaiseEvent` in each Sub
- UI: Code Navigator **Show Causal Chain** button; Context Rail preview in event handlers
- Tests: `tests/test_vg_causal_chain.gd` (8 fixtures)
