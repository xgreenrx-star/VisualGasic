...
| **M7+ — Performance Optimizations (Research)** | December 2026+ | (1) **Type-Tagged Locals**: Specialize bytecode VM to use fast-path int64/double registers for variables declared `As Integer`/`As Long`/`As Double`, avoiding Variant dispatch overhead on every arithmetic op. Expected: 2-5× speedup on numeric loops. (2) **On-Stack Replacement (OSR)**: JIT tier-2 optimization: detect hot loops running N iterations, pause, compile a specialized trace, jump into compiled code. Expected: 3-10× speedup on long-running loops. (3) **String Arena**: Thread-local buffer for chained `&` concatenation to eliminate intermediate allocations. Expected: 1.5-3× speedup on StringConcat. Research phase; target implementation Q1 2027. |
