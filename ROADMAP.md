...
| **Sub/Function Call Overhead Reduction** | **CONFIRMED CRITICAL (Jul 29 2026)** — root-caused and quantified via a new `BenchCall`/`FunctionCall` entry added to the built-in benchmark suite (`demo/bench.vg` + `demo/test_suites/run_benchmarks.gd`, mirrors the existing Arithmetic/ArraySum/etc. harness): 50,000 trivial one-line Function calls take GDScript 6.9ms vs **VG 3.17 seconds — VG is 458× SLOWER than GDScript for pure call overhead**, the only benchmark in the entire suite where VG loses this badly (every other benchmark shows VG winning 10-70×). Root cause identified in `visual_gasic_instance_call.inc::call_internal()` (the shared dispatcher used by BOTH the tree-walk interpreter AND the bytecode VM's `OP_CALL` — bytecode compilation speeds up a Sub's own body but every CALL to another Sub/Function still funnels through this same expensive path): every single call (a) linearly scans all Subs in the module doing case-insensitive string comparisons to resolve the callee by name (no cache), (b) does a second linear scan for overload-mangling, (c) allocates and populates a heap-backed `Dictionary saved_locals` (hashes+inserts every param/local name) for save/restore bookkeeping regardless of whether recursion is even possible, (d) does a string-keyed bytecode-chunk lookup. None of this is cached per call-site or amortized across calls. | Fix approach TBD — candidates: (1) cache resolved `SubDefinition*`/bytecode-chunk pointers keyed by call-site (biggest low-risk win, avoids the two linear scans + repeated string-keyed lookups), (2) replace `Dictionary saved_locals` with a pre-sized `Vector<Variant>`/array-based frame using the already-available `local_names` index positions instead of hashing string keys per call. Est. HIGH effort or unknown pending a scoped profiling spike of `call_internal()` in isolation. | **HIGH — real, hard-measured, hurts ANY call-heavy VG program** (emulators, AST walkers, state machines, recursive algorithms), not a niche demo issue. See `/memories/repo/vg_bytecode_perf.md` for full measurement + root-cause writeup. |
