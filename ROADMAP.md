...
| **Sub/Function Call Overhead Reduction** (research) | Found live (Jul 29 2026) profiling `demos/C64_Emulator`'s `C64_Step()` (a module-level Function bytecode-compiled via the M6 cross-module fix): even a call chain of only 1-2 nested Sub/Function calls per invocation costs a large, measurable fraction of total time — a synthetic NOP-only benchmark improved ~40% (1408→1982 steps/sec, same-machine A/B) purely by removing ONE unconditional per-call helper invocation from the hot path (no opcode-dispatch changes). Suggests the per-call fixed cost (locals-frame setup, param binding/`saved_locals` bookkeeping in `call_internal()`) is disproportionately expensive relative to the work done inside tiny helper Functions — exactly the pattern hit by any interpreter/emulator-style VG program (CPU cores, AST walkers, state machines) that factors hot logic into many small helper calls. Needs profiling of `call_internal()`/`visual_gasic_instance_call.inc` to quantify the fixed per-call cost in isolation (independent of Type-Tagged Locals, which addresses Variant dispatch, not call setup) before committing to an approach (candidates: avoid re-allocating locals Dictionary/Vector per call, cache param-binding metadata, fast-path for calls with no `ByRef` params). | Unknown until profiled — first-instruction-of-a-call cost reduction would benefit EVERY VG program with any Sub/Function calls, not just tight loops | Unknown (research spike needed first: instrument `call_internal()` with a call-count/cumulative-time counter, isolate fixed cost from argument-count-dependent cost) | **Flagged for future investigation** — not scheduled against a milestone yet; raised from real-world C64 emulator perf work, see `/memories/repo/vg_bytecode_perf.md` |
