...
**Current limitation**: VG `Array()` stores all numbers as floats, so integer-typed numpy args (`zeros(5)`, `eye(3)`, `linspace(0,1,5)`) fail with `TypeError: 'float' object cannot be interpreted as an integer`. **Phase 1 is the proper fix** — a typed binary protocol that sends int/float/string/bool type information alongside each value, so numpy receives actual Python `int` arguments when VG sends integers.
