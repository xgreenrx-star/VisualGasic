...
void VisualGasicInstance::execute_statement(Statement* stmt) {
    ...
    switch (stmt->type) {
        ...
        case STMT_WHENEVER_SECTION: {
            ...
            section.callback_procedure = s->callback_procedure;
...
void VisualGasicInstance::check_whenever_conditions(const String& variable_name, const Variant& new_value) {
    for (int i = 0; i < whenever_sections.size(); i++) {
        ...
        if (condition_met) {
            ...
            Array empty_args;
            bool found = false;
            call_internal(section.callback_procedure, empty_args, found);
            
            if (!found) {
                UtilityFunctions::print("Warning: Whenever callback procedure '", section.callback_procedure, "' not found");
            }
        } else {
...
String VisualGasicInstance::get_whenever_status() const {
    ...
    for (int i = 0; i < whenever_sections.size(); i++) {
        ...
        status += "- " + section.section_name + " (" + section.variable_name + " " + 
                 section.comparison_operator + ") -> " + section.callback_procedure + " [" + state + "]\n";
    }
