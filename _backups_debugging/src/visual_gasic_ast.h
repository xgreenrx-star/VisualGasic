...
struct GenericTypeParameter {
    String name;
    Vector<String> constraints; // Where T : IComparable, ICloneable
...
struct AdvancedType {
    enum TypeKind { BASIC, GENERIC, OPTIONAL, UNION, ARRAY };
    
    TypeKind kind;
    String base_type;
    Vector<AdvancedType*> type_parameters; // For generics like List(Of T)
    Vector<AdvancedType*> union_types; // For String | Integer | Boolean
    bool is_optional; // For Player?
    
    AdvancedType() : kind(BASIC), is_optional(false) {}
    ~AdvancedType() {
        for(int i=0; i<type_parameters.size(); i++) if(type_parameters[i]) delete type_parameters[i];
        for(int i=0; i<union_types.size(); i++) if(union_types[i]) delete union_types[i];
    }
};

// Pattern Matching Structures
struct Pattern {
    enum PatternType { LITERAL_PATTERN, VARIABLE_PATTERN, TYPE_PATTERN, TUPLE_PATTERN, GUARD_PATTERN };
...
struct PatternMatchStatement : Statement {
    ExpressionNode* expression;
    Vector<MatchCase*> cases;
    
    PatternMatchStatement() : Statement(STMT_PATTERN_MATCH) { expression = nullptr; }
    ~PatternMatchStatement() {
        if (expression) delete expression;
        for(int i=0; i<cases.size(); i++) if(cases[i]) delete cases[i];
    }
};
...
struct TypeCheckExpression : ExpressionNode {
    ExpressionNode* expression;
    AdvancedType* check_type;
