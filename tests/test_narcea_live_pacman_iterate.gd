...
const PROMPT_ITERATE := (
	"Good job but lets make the maze too. Add special power-ups in the corners "
	+ "that makes the bad guys run from the player and if the player catches one "
	+ "while the power up is active the ghost will disappear, wait a while then "
	+ "reappear as a normal bad guy. The power-ups should only last a short period "
	+ "of time maybe 20 seconds."
)
...
func _initialize() -> void:
	print("=== Narcea Live Pacman Iterate ===")
	...
	print("")
	print("--- Turn 1: create pacman ---")
	call_deferred("_start_gemini", _hardened_create(PROMPT_CREATE))
...
func _on_http_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	if _phase == 1:
		...
		print("")
		print("--- Turn 2: add maze + power-ups ---")
		_start_gemini(_hardened_iterate(PROMPT_ITERATE))
		_deadline_ms = Time.get_ticks_msec() + int(TIMEOUT_SEC * 1000.0)
		_phase = 2
		return
	...
	if _phase == 2:
		...
		var maze_hits := 0
		for kw in ["maze", "wall", "power", "fright", "corner", "grid"]:
			if low.find(kw) >= 0:
				maze_hits += 1
		_expect("turn2 maze/power keywords", maze_hits >= 2, "hits=%d body_len=%d" % [maze_hits, vg_after.length()])
		_cleanup(ROOT)
		_finish()
...
func _test_offline_intent() -> void:
	...
	var intent1: String = panel._detect_build_intent(PROMPT_CREATE)
	_expect("create prompt -> project", intent1 == "project", "got %s" % intent1)
	var intent2: String = panel._detect_build_intent(PROMPT_ITERATE)
	_expect("iterate prompt -> project", intent2 == "project", "got %s" % intent2)
	_expect("prompt_iterates_project", panel._prompt_iterates_project(PROMPT_ITERATE))
	panel.set("_last_project_root", "")
	panel.set("_last_run_scene", "")
	var intent3: String = panel._detect_build_intent(PROMPT_ITERATE)
...
func _hardened_create(desc: String) -> String:
	return (
		...
		+ "with _Ready/_Process/_Draw using DrawRect/DrawCircle only. Keep ≤ 4 files. "
...
func _hardened_iterate(desc: String) -> String:
	return (
		...
		+ "Include files[] with the FULL updated Game.vg source (grid maze with walls, corner power-ups, "
		+ "frightened ghosts for ~20 seconds, eaten ghosts respawn). "
...
func _scaffold(response: String, label: String) -> void:
	...
	ProjectSynth.finalize_project(spec, ps.project_root(spec), PROMPT_CREATE if label == "turn1" else PROMPT_ITERATE)
