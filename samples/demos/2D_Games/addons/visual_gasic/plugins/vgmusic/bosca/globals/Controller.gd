...
const MAIN_WINDOW_SCRIPT := preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/MainWindow.gd")
const INFO_POPUP_SCENE := preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/widgets/popups/InfoPopup.tscn")
...
var instrument_themes: Dictionary = {
	BoscaColorPalette.PALETTE_BLUE:   preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_blue.tres"),
	BoscaColorPalette.PALETTE_PURPLE: preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_purple.tres"),
	BoscaColorPalette.PALETTE_RED:    preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_red.tres"),
	BoscaColorPalette.PALETTE_ORANGE: preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_orange.tres"),
	BoscaColorPalette.PALETTE_GREEN:  preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_green.tres"),
	BoscaColorPalette.PALETTE_CYAN:   preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_cyan.tres"),
	BoscaColorPalette.PALETTE_GRAY:   preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/theme/instruments/instrument_theme_gray.tres"),
}
...
# ─── VGMusic embedding ──────────────────────────────────────────────
# When embedded inside the VisualGasic IDE, Bosca's Controller autoload is
# loaded at engine startup but MUST NOT do its heavy initialization until
# the user actually opens the VGMusic tab — otherwise it would hijack the
# main window's script, override quit handling, and instantiate the audio
# driver in projects that aren't using the music maker.
#
# vg_vgmusic_plugin.gd calls Controller.vgmusic_boot() on activation.
var vgmusic_booted: bool = false


func _init() -> void:
	# Eager initialization disabled — see vgmusic_boot().
	pass


## Called by vg_vgmusic_plugin.gd when the user activates the VGMusic tab.
## Performs the work that Bosca's original _init() and _ready() did.
func vgmusic_boot() -> void:
	if vgmusic_booted:
		return
	vgmusic_booted = true

	_vgmusic_register_input_actions()
	...
	# Skip get_window().set_script(MAIN_WINDOW_SCRIPT): the host VG IDE
	# owns the OS-level window. The plugin instead applies MainWindow
	# behavior to its embedded view if it needs window-level handlers.
	# Skip set_auto_accept_quit(false): VG IDE already manages quit.
	music_player.initialize_driver()


func _vgmusic_register_input_actions() -> void:
	# Bosca Ceoil Blue ships these in its own project.godot. When embedded
	# under VG, register them at runtime so the host project doesn't need
	# to know about them.
	var defs: Array = [
		# [action, keycode, ctrl, shift, alt]
		["bosca_playstop",                 KEY_SPACE,  false, false, false],
		["bosca_pause",                    KEY_SPACE,  true,  false, false],
		["bosca_new",                      KEY_N,      true,  false, false],
		["bosca_open",                     KEY_O,      true,  false, false],
		["bosca_save",                     KEY_S,      true,  false, false],
		["bosca_save_as",                  KEY_S,      true,  false, true ],
		["bosca_exit",                     KEY_ESCAPE, false, false, false],
		["bosca_toggle_fullscreen",        KEY_F,      true,  false, false],
		["bosca_notemap_up",               KEY_UP,     false, false, false],
		["bosca_notemap_down",             KEY_DOWN,   false, false, false],
		["bosca_notemap_cursor_bigger",    KEY_RIGHT,  true,  false, false],
		["bosca_notemap_cursor_smaller",   KEY_LEFT,   true,  false, false],
		["bosca_patternmap_left",          KEY_LEFT,   false, false, false],
		["bosca_patternmap_right",         KEY_RIGHT,  false, false, false],
		["bosca_patternmap_scale_bigger",  KEY_PLUS,   true,  false, false],
		["bosca_patternmap_scale_smaller", KEY_MINUS,  true,  false, false],
		["bosca_patternmap_duplicate",     KEY_D,      true,  false, false],
		["bosca_debug_1",                  KEY_F11,    true,  false, false],
	]
	for d in defs:
		var name: StringName = StringName(d[0])
		...
		ev.keycode = d[1]
		ev.ctrl_pressed = d[2]
		ev.shift_pressed = d[3]
		ev.alt_pressed = d[4]
		InputMap.action_add_event(name, ev)


func _ready() -> void:
	# No-op until vgmusic_boot() is called.  See above.
	pass


func _notification(what: int) -> void:
	if not vgmusic_booted:
		# Don't intercept window/quit notifications while the plugin
		# is dormant — those belong to the VG IDE.
		return
