...
const CREDITS_POPUP_SCENE := preload("res://addons/visual_gasic/plugins/vgmusic/bosca/gui/widgets/popups/CreditsPopup.tscn")
...
func _update_version_flair() -> void:
	...
	var version_setting: String = str(ProjectSettings.get_setting("application/config/version", "1.0"))
	var flair_setting: String = str(ProjectSettings.get_setting("application/config/version_flair", ""))
	
	var version_arr := version_setting.split(".")
	var major := version_arr[0] if version_arr.size() > 0 else "1"
	var minor := version_arr[1] if version_arr.size() > 1 else "0"
	var version_string := "v%s.%s" % [ major, minor ]
