...
out_vec="$(timeout 60 "$GODOT" --headless --path "$HOST" -s "$ROOT/tests/test_vector_data_resolver.gd" 2>&1)" || ec_vec=$?
echo "$out_vec"
if ! echo "$out_vec" | grep -q "RESULTS:.*0 failed"; then
	if [[ "${ec_vec:-0}" -ne 0 ]]; then
		exit 1
	fi
	exit 1
fi
